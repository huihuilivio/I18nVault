...
size_t sm4_cbc_encrypt(const sm4_ctx* ctx, uint8_t iv[SM4_BLOCK_SIZE], const uint8_t* in, size_t in_len, uint8_t* out)
{
    ...
    {
        size_t copy = (off + SM4_BLOCK_SIZE <= in_len) ? SM4_BLOCK_SIZE : in_len - off;
        memcpy(block, in + off, copy);
        /* PKCS#7: fill remaining bytes with pad value */
        if (copy < SM4_BLOCK_SIZE)
            memset(block + copy, (uint8_t)pad, SM4_BLOCK_SIZE - copy);
        for (int i = 0; i < SM4_BLOCK_SIZE; i++)
            block[i] ^= iv[i];
        sm4_encrypt_block(ctx, block, out + off);
        memcpy(iv, out + off, SM4_BLOCK_SIZE);
    }
    return total;
}

size_t sm4_cbc_decrypt(const sm4_ctx* ctx, uint8_t iv[SM4_BLOCK_SIZE], const uint8_t* in, size_t in_len, uint8_t* out)
{
    ...
    for (size_t off = 0; off < in_len; off += SM4_BLOCK_SIZE)
    {
        ...
        memcpy(iv, in + off, SM4_BLOCK_SIZE);
        sm4_decrypt_block(ctx, in + off, out + off);
        for (int i = 0; i < SM4_BLOCK_SIZE; i++)
            out[off + i] ^= prev[i];
    }
