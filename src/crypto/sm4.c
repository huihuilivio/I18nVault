...
/* T = L(τ(A)) */
static uint32_t T_enc(uint32_t A) { return L(tau(A)); }

/* T' = L'(τ(A)) */
static uint32_t T_key(uint32_t A) { return L_prime(tau(A)); }

/* ---- Core round (shared by encrypt / decrypt) ---- */

static void sm4_rounds(const uint32_t rk[32],
                       const uint8_t in[16],
                       uint8_t out[16]) {
  uint32_t X[36];
  X[0] = get_be32(in);
...
size_t sm4_cbc_encrypt(const sm4_ctx *ctx, uint8_t iv[SM4_BLOCK_SIZE],
  ...
  for (size_t off = 0; off < total; off += SM4_BLOCK_SIZE) {
    size_t copy = (off + SM4_BLOCK_SIZE <= in_len)
                      ? SM4_BLOCK_SIZE
                      : in_len - off;
